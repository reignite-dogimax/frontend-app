...
export class AppointmentsStore {
   ...
   * @returns Observable that completes when the appointment is created
   */
   ...
   * @returns Observable that completes when the appointment is updated
   */
  updateAppointment(updatedAppointment: Appointment): Observable<Appointment> {
    this.loadingSignal.set(true);
    this.errorSignal.set(null);
    return this.appointmentsApi.updateAppointment(updatedAppointment).pipe(
      retry(2),
      tap({
        next: appointment => {
   ...
   * @returns Observable that completes when the appointment is deleted
   */
  deleteAppointment(id: number): Observable<void> {
    this.loadingSignal.set(true);
    this.errorSignal.set(null);
    return this.appointmentsApi.deleteAppointment(id).pipe(
      retry(2),
      tap({
        next: () => {
   ...
   * @returns Observable that completes when the veterinary is created
   */
  addVeterinary(veterinary: Veterinary): Observable<Veterinary> {
    this.loadingSignal.set(true);
    this.errorSignal.set(null);
    return this.appointmentsApi.createVeterinary(veterinary).pipe(
      retry(2),
      tap({
   ...
   * @returns Observable that completes when the veterinary is updated
   */
  updateVeterinary(updatedVeterinary: Veterinary): Observable<Veterinary> {
    this.loadingSignal.set(true);
    this.errorSignal.set(null);
    return this.appointmentsApi.updateVeterinary(updatedVeterinary).pipe(
      retry(2),
      tap({
        next: veterinary => {
   ...
   * @returns Observable that completes when the veterinary is deleted
   */
  deleteVeterinary(id: number): Observable<void> {
    this.loadingSignal.set(true);
    this.errorSignal.set(null);
    return this.appointmentsApi.deleteVeterinary(id).pipe(
      retry(2),
      tap({
        next: () => {
